    ...
    class CrimeHolder extends RecyclerView.ViewHolder {
        ...
        void bind(Crime crime, OnCrimeClickListener listener) {
            try {
                ...
                // Change background color based on solved state
                if (itemView != null) {
                    if (crime != null && crime.isSolved()) {
                        // light green when solved
                        itemView.setBackgroundColor(0xFFCCFFCC);
                    } else {
                        // transparent/default when not solved
